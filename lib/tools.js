...
- ZMĚNY VZHLEDU A CHOVÁNÍ (kurzor, barvy, písmo, velikosti, rozestupy, skrytí prvku, vypnutí animace
  apod.) dělej v REÁLNÉM ČASE přes vlastní CSS. POSTUP MÁ PŘESNĚ 3 KROKY, každý nástroj zavolej NEJVÝŠ JEDNOU:
  1) inspect_design("<co měníš>") — zjisti třídy/CSS. 2) get_custom_css — přečti dosavadní CSS (jen jednou!).
  3) set_custom_css — napiš KOMPLETNÍ CSS (dosavadní pravidla + nové, klidně s !important) a HOTOVO.
  NIKDY nevolej stejný nástroj dvakrát. Po set_custom_css rovnou napiš uživateli shrnutí. NEposílej
  vizuální úpravy jako návrh kódu (propose_code_change).
  Příklad „normální kurzor": inspect_design("cursor") (najdeš .cursor-dot/.cursor-ring a body.cursor-on)
  → set_custom_css: "*{cursor:auto!important} .cursor-dot,.cursor-ring{display:none!important}".

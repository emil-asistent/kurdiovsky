...
export async function runAgent({ history = [], userText, ctx = {}, onStep }) {
  ...
  for (let i = 0; i < MAX_ITER; i++) {
    const force = reads >= READ_CAP; // po stropu: nabídnout JEN akční nástroje (bez 'required' — DeepSeek/Cloudflare ho nepodporuje)
    const resp = await callOpenRouter(convo, force ? FORCE_TOOLS : TOOLS, 'auto');

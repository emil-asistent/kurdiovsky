...
const ACTION_TOOLS = TOOLS.filter((t) => !READ_TOOLS.has(t.function?.name));
const READ_CAP = 4;
...
async function callOpenRouter(messages, tools = TOOLS) {
...
export async function runAgent({ history = [], userText, ctx = {}, onStep }) {
  ...
  for (let i = 0; i < MAX_ITER; i++) {
    const resp = await callOpenRouter(convo, reads >= READ_CAP ? ACTION_TOOLS : TOOLS);

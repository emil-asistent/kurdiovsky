...
// Skutečné CSS webu (z <style> bloků index.html) — dáváme agentovi do kontextu, ať píše přesné selektory.
let siteCssCache = null;
export async function getSiteCss() {
  if (siteCssCache !== null) return siteCssCache;
  try {
    const s = await fs.readFile(path.join(WEBROOT, 'index.html'), 'utf8');
    const blocks = [...s.matchAll(/<style[^>]*>([\s\S]*?)<\/style>/gi)].map((m) => m[1]);
    siteCssCache = blocks.join('\n').slice(0, 28000);
  } catch { siteCssCache = ''; }
  return siteCssCache;
}
